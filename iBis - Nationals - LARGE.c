...
task main()
{
	...
	while(true)
	{
		...
		{
			moveSpeed = 0; // The movemenr speed is set to 0
		}
		else
		{
			if (abs(ballAngle) < 20) // If the angle of the ball is less than 20
			{
				moveAngle = ballAngle * 0.5; // Go towards the ball (with tightener)
			}

			if (abs(ballAngle) <= 70) // If ballAngle is between 20 & 70
			{
				moveAngle = ballAngle + multi(ballStrength) * (35 * sign(ballAngle)); // Orbit around the ball(45)
			}
			else // If ballAngle >  70
			{
				moveAngle = ballAngle + multi(ballStrength) * (70 * sign(ballAngle));// Orbit around the ball(90)
			}
		}
